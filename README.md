...
### 2. MQTT 브로커 인증/ACL을 백엔드 API로 위임

- **문제**: Mosquitto 자체 파일 기반 인증은 사용자 추가/권한 변경 시 브로커 재시작이 필요하고, 사용자/디바이스 관리가 백엔드 DB와 분리되어 운영 부담이 큼.
- **해결 과정**:
    - Mosquitto의 `mosquitto-go-auth` 플러그인을 사용해 HTTP Backend로 백엔드 API 연동
    - 백엔드의 `/mqtt/auth`, `/mqtt/acl` 엔드포인트가 사용자/허브/토픽 권한을 단일 진실 공급원(Single Source of Truth)으로 검증
    - 토픽 패턴(`chome/{hubId}/+`)에 대해 사용자–허브 권한 매핑을 검사
- **결과**: 사용자/디바이스 권한 변경이 브로커 재시작 없이 즉시 반영. 인증/인가 정책을 백엔드 비즈니스 로직과 일관되게 관리.
